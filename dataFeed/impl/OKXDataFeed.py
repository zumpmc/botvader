...
class OKXDataFeed(DataFeed):
    """Streams BTC-USDT trades from OKX and publishes batched ticks to S3
    ...
    def _flush(self, window_end: Optional[float] = None) -> None:
        ...
        key = f"okx/{self.name}/{window_start:.6f}-{window_end:.6f}"
    ...
    def _on_message(self, ws, message: str) -> None:
        ...
        # OKX trade messages have "data" array under the "trades" channel
        ...
        for trade in raw.get("data", []):
            ...
            try:
                trade_ts = float(trade["ts"]) / 1000.0
            except (KeyError, ValueError, TypeError):
                trade_ts = time.time()
